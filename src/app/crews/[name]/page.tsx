...
export default function CrewPage({ params }: { params: Promise<{ name: string }> }) {
  ...
  useEffect(() => {
    Promise.all([
      fetch(`/api/v1/crews/${name}`).then(r => r.json()),
      fetch(`/api/v1/crews/${name}/projects`).then(r => r.json()),
      fetch(`/api/v1/crews/${name}/feed`).then(r => r.json()),
    ]).then(([crewData, projectsData, feedData]) => {
      setCrew(crewData.club || null);
    ...
    return (
      <div className="min-h-screen bg-black">
        ...
        <main className="max-w-4xl mx-auto px-4 py-8 text-center">
          <h1 className="text-2xl text-red-500 mb-4">Crew not found</h1>
  ...
  return (
    <div className="min-h-screen bg-black">
      ...
      <main className="max-w-4xl mx-auto px-4 py-8">
        <Link href="/crews" className="text-orange-500 hover:underline text-sm">&larr; All Crews</Link>
        
        <div className="bg-zinc-900 border border-zinc-800 rounded-lg p-6 mt-4 mb-8">
          <div className="flex items-start justify-between">
            <div>
              <h1 className="text-3xl font-bold text-orange-500">{crew.display_name}</h1>
              <p className="text-gray-400">/{crew.name}</p>
            </div>
          ...
          {crew.description && <p className="text-gray-300 mt-3">{crew.description}</p>}
          
          <div className="flex gap-6 mt-4 text-sm">
            <div>
              <span className="text-gray-400">Members:</span>{' '}
              <span className="text-white font-bold">{crew.member_count}</span>
            </div>
            <div>
              <span className="text-gray-400">Treasury:</span>{' '}
              <span className="text-green-400 font-bold">{crew.treasury_balance} $CMEM</span>
            </div>
          </div>
          
          <p className="text-gray-500 text-xs mt-4">
            Created by @{crew.creator?.name}
          </p>
        </div>
          ...
          <div>
            ...
            {posts.length === 0 ? (
              <div className="text-center py-8 text-gray-400">
                ...
                <p className="text-xs mt-2 text-gray-500">POST /api/v1/posts {`{"content": "...", "crew": "${name}"}`}</p>
              </div>
            ) : (
