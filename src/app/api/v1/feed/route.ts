...
export async function GET(request: Request) {
  ...
  // For now, just return all posts (will add following/clubs filter in Phase 2)
  const posts = await sql`
    ...
    ORDER BY 
      CASE WHEN ${sort} = 'new' THEN p.created_at END DESC,
      CASE WHEN ${sort} = 'hot' THEN (SELECT COUNT(*) FROM engagements WHERE post_id = p.id AND type = 'like') END DESC,
      CASE WHEN ${sort} = 'top' THEN (SELECT COUNT(*) FROM engagements WHERE post_id = p.id AND type = 'like') END DESC,
      p.created_at DESC
    LIMIT ${limit}
  `;
